...
sys.path.insert(0, str(Path(__file__).resolve().parent / 'chromium-build-system' / 'utils'))
import downloads
...
def main():
    ...
    if not args.ci or not (source_tree / 'BUILD.gn').exists():
        ...
        download_info = downloads.DownloadInfo([
            ...
            _ROOT_DIR / 'chromium-build-system' / 'downloads.ini',
        ])
        ...
        unremovable_files = prune_binaries.prune_files(
            source_tree,
            (_ROOT_DIR / 'chromium-build-system' / 'pruning.list').read_text(encoding=ENCODING).splitlines()
        )
        ...
        patches.apply_patches(
            patches.generate_patches_from_series(_ROOT_DIR / 'chromium-build-system' / 'patches', resolve=True),
            source_tree,
        ...
        domain_substitution.apply_substitution(
            _ROOT_DIR / 'chromium-build-system' / 'domain_regex.list',
            _ROOT_DIR / 'chromium-build-system' / 'domain_substitution.list',
            source_tree,
            None
        )
    ...
    if not args.ci or not (source_tree / 'out/Default').exists():
        ...
        gn_flags = (_ROOT_DIR / 'chromium-build-system' / 'flags.gn').read_text(encoding=ENCODING)
        gn_flags += '\n'
